jQuery(document).ready(function() {
	...
	jQuery(document).on('click', 'input[name=zopimapi_edit_restrict]', function(){
	...
	if (jQuery('input[name=zopimapi_edit_restrict]:checked').val() == 'roles' ){
			jQuery('p.restrict_roles').show( function(){
				jQuery("p.restrict_roles select").removeClass("chzn-done").css('display', 'inline').data('chosen', null);
				jQuery("p.restrict_roles *[class*=chzn], p.restrict_roles .chosen-container").remove();
				jQuery("p.restrict_roles .chosen-select").chosen({
	...
	if ( jQuery('div.zopimapi-admin h3').length <= 3) {
		jQuery('div.zopimapi-admin h3:first').addClass('selected');
		jQuery('div.zopimapi-admin h3:first').next('table.form-table, .upadmin-panel').show();
		jQuery('table[data-type=conditional]').hide();
		jQuery('table[rel=' + jQuery('#badge_method').val() + ']').show();
		jQuery(".zopimapi-admin select").removeClass("chzn-done").css('display', 'inline').data('chosen', null);
		jQuery(".zopimapi-admin *[class*=chzn], .zopimapi-admin .chosen-container").remove();
		jQuery(".zopimapi-admin .chosen-select").chosen({
			disable_search_threshold: 10
		});
	}
	
	/* Expand table under h3 */
	jQuery(document).on('click', 'div.zopimapi-admin h3:not(.selected)', function(){
	...
	jQuery(document).on('click', 'div.zopimapi-admin h3.selected', function(){
	...
	jQuery(document).on('click', '.upadmin-user-deny', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_admin_user_deny&user_id=' + jQuery(this).data('user'),
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
				...
				jQuery('.toplevel_page_zopimapi').find('span.update-count').html( data.count );
			}
		});
		return false;
	});
	...
	jQuery(document).on('click', '.upadmin-user-approve', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_admin_user_approve&user_id=' + jQuery(this).data('user'),
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
				parent.fadeOut();
	...
	jQuery(document).on('click', '.upadmin-verify-u', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_verify_user&user_id=' + link.data('user'),
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
	...
	jQuery(document).on('click', '.upadmin-unverify-u', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_unverify_user&user_id=' + link.data('user'),
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
				parent.hide().html( data.admin_tpl ).fadeIn();
	...
	jQuery(document).on('click', '.upadmin-invite-u', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_verify_invite&user_id=' + link.data('user'),
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
				parent.hide().html( data.admin_tpl ).fadeIn();
	...
	jQuery(document).on('click', '.upadmin-verify', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_verify_user&user_id=' + jQuery(this).data('user'),
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
				parent.fadeOut();
	...
	jQuery(document).on('click', '.upadmin-unverify', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_unverify_user&user_id=' + jQuery(this).data('user'),
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
				parent.fadeOut();
	...
	if (jQuery('#dashboard_redirect_users').val()==2){
		jQuery('#dashboard_redirect_users').parents('td').find('.zopimapi-admin-hide-input').css({'display':'block'});
	}
	if (jQuery('#profile_redirect_users').val()==2){
		jQuery('#profile_redirect_users').parents('td').find('.zopimapi-admin-hide-input').css({'display':'block'});
	}
	jQuery('#dashboard_redirect_users,#profile_redirect_users').change(function(){
		if (jQuery(this).val() == 2) {
			jQuery(this).parents('td').find('.zopimapi-admin-hide-input').css({'display':'block'});
		} else {
			jQuery(this).parents('td').find('.zopimapi-admin-hide-input').css({'display':'none'});
		}
	});
	...
	jQuery(document).on('click', '#upadmin-sortable-fields .upadmin-field-actions a', function(e){
		...
		if (act == 'upadmin-field-action-remove') {
			...
			jQuery.ajax({
				url: ajaxurl,
				data: 'action=zopimapi_delete_field&field=' + field,
				dataType: 'JSON',
				type: 'POST',
				success:function(data){
	...
	jQuery(document).on('change', '#upadmin-sortable-fields .upadmin-field-zone input, #upadmin-sortable-fields .upadmin-field-zone select, #upadmin-sortable-fields .upadmin-field-zone textarea', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_update_field&field=' + field + str,
			//dataType: 'JSON',
			type: 'POST',
			success:function(data){
				load.removeClass('loading');
	...
	jQuery(document).on('click', '.upadmin-reset-fields', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_restore_builtin_fields',
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
	...
	jQuery(document).on('click', '.upadmin-reset-groups', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: 'action=zopimapi_restore_builtin_groups',
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
				form.find('.upadmin-loader').removeClass('loading');
	...
	jQuery(document).on('submit', '.upadmin-new form', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: form.serialize() + '&action=zopimapi_create_field',
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
	...
	jQuery(document).on('click', '.upadmin-tpl a.resetgroup', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: form.serialize() + '&action=zopimapi_reset_group&role='+role,
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
	...
	jQuery(document).on('submit', '.upadmin-tpl form', function(e){
		...
		jQuery.ajax({
			url: ajaxurl,
			data: form.serialize() + '&action=zopimapi_save_group&role='+role+'&group='+group,
			dataType: 'JSON',
			type: 'POST',
			success:function(data){
				form.find('.upadmin-miniload').remove();
